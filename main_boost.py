...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=[1, 2, 3],
        default=None,
        help='Execute specific phase only: 1=LLM Config, 2=Statistical Computing, 3=LLM Post-processing+Report'
    )
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Skip LLM revision in Phase 2 (useful for offline execution)'
    )
...
def main(args):
    ...
    if start_phase <= 1:
        ...
        if args.phase == 1:
            logger.checkpoint("🎉 Phase 1 completed! Run Phase 2 with: --resume-from output/Abalone/phase1_checkpoint.pkl --phase 2")
    ...
    #######################################################################################
        ...
        if args.phase == 2:
            logger.checkpoint("🎉 Phase 2 completed! Run Phase 3 with: --resume-from output/Abalone/phase2_checkpoint.pkl --phase 3")
            return None, global_state
    ########################################################################################
    if start_phase <= 3:
        logger.header("🟡 PHASE 3: LLM Post-processing & Report Generation")
        
        # ========== 如果Phase 2跳过了LLM revision，这里执行 ==========
        if args.skip_llm_revision or not hasattr(global_state.results, 'revised_graph'):
            logger.section("Graph Revision (LLM)")
            logger.detail("Applying LLM-based graph revision...")
            judge = Judge(global_state, args)
            global_state = judge.llm_revision(global_state, 'cot_all_relation', 1)
