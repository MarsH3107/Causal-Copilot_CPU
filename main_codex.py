...
##@@ -76,50 +79,72 @@ def parse_args():
    ...
    parser.add_argument(
        '--stage',
        type=str,
        choices=['full', 'prepare', 'execute', 'refine'],
        default='full',
        help='Run a specific stage of the pipeline (full, prepare, execute, refine)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Path to a previously saved global state to resume from'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='File path to save the global state after the stage completes'
    )
...
##@@ -132,260 +157,462 @@ def process_user_query(query, data):
...
def save_global_state(global_state, path):
    os.makedirs(os.path.dirname(path), exist_ok=True) if os.path.dirname(path) else None
    with open(path, 'wb') as f:
        pickle.dump(global_state, f)
    logger.success(f"Global state saved to {path}")


def load_global_state_from_file(path):
    with open(path, 'rb') as f:
        state = pickle.load(f)
    logger.detail(f"Loaded global state from {path}")
    return state


def save_global_state_to_json(global_state, stage_name=None):
    """Persist the global state to both pickle and JSON artifacts."""
    output_dir = Path(global_state.user_data.output_graph_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    algo_name = getattr(global_state.algorithm, 'selected_algorithm', None) or 'unknown_algorithm'
    suffix = f"{algo_name}_{stage_name}" if stage_name else algo_name

    pkl_path = output_dir / f'{suffix}_global_state.pkl'
    ...
    knowledge_docs = getattr(global_state.user_data, 'knowledge_docs', None)
    if isinstance(knowledge_docs, str):
        knowledge_docs_serializable = [knowledge_docs]
    else:
        knowledge_docs_serializable = knowledge_docs
    ...
    data_to_save = {
        ...
        'stage': stage_name,
        'user_data': {
            'initial_query': getattr(global_state.user_data, 'initial_query', None),
            'knowledge_docs': knowledge_docs_serializable,
            'knowledge_docs_for_user': getattr(global_state.user_data, 'knowledge_docs_for_user', None),
        },
        'statistics': {
            'sample_size': getattr(global_state.statistics, 'sample_size', None),
            'feature_number': getattr(global_state.statistics, 'feature_number', None),
            'data_type': getattr(global_state.statistics, 'data_type', None),
            'linearity': getattr(global_state.statistics, 'linearity', None),
            'gaussian_error': getattr(global_state.statistics, 'gaussian_error', None),
            'missingness': getattr(global_state.statistics, 'missingness', None),
            'heterogeneous': getattr(global_state.statistics, 'heterogeneous', None),
            'time_series': getattr(global_state.statistics, 'time_series', None),
            'description': getattr(global_state.statistics, 'description', None),
        },
        'algorithm_info': {
            'selected_algorithm': getattr(global_state.algorithm, 'selected_algorithm', None),
            'selected_reason': getattr(global_state.algorithm, 'selected_reason', None),
            'algorithm_candidates': getattr(global_state.algorithm, 'algorithm_candidates', None),
            'algorithm_arguments': getattr(global_state.algorithm, 'algorithm_arguments', None),
            'algorithm_optimum': getattr(global_state.algorithm, 'algorithm_optimum', None),
            'hyperparameters': getattr(global_state.algorithm, 'hyperparameters', None),
        },
        'results': {
            'converted_graph': convert_to_serializable(getattr(global_state.results, 'converted_graph', None)),
            'revised_graph': convert_to_serializable(getattr(global_state.results, 'revised_graph', None)),
            'lagged_graph': convert_to_serializable(getattr(global_state.results, 'lagged_graph', None)),
            'bootstrap_probability': convert_to_serializable(getattr(global_state.results, 'bootstrap_probability', None)),
            'bootstrap_check_dict': convert_to_serializable(getattr(global_state.results, 'bootstrap_check_dict', None)),
            'llm_errors': convert_to_serializable(getattr(global_state.results, 'llm_errors', None)),
            'raw_edges': convert_to_serializable(getattr(global_state.results, 'raw_edges', None)),
            'revised_edges': convert_to_serializable(getattr(global_state.results, 'revised_edges', None)),
        },
        'logging': {
            'select_conversation': getattr(global_state.logging, 'select_conversation', None),
            'argument_conversation': getattr(global_state.logging, 'argument_conversation', None),
            'knowledge_conversation': getattr(global_state.logging, 'knowledge_conversation', None),
            'graph_conversion': getattr(global_state.logging, 'graph_conversion', None),
        },
    }

    json_path = output_dir / f'{suffix}_complete_information.json'
    try:
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(convert_to_serializable(data_to_save), f, indent=2, ensure_ascii=False)
        logger.success(f"Saved complete information to: {json_path}")
    except Exception as exc:
        logger.warning(f"Failed to save JSON (non-critical): {exc}")

    if knowledge_docs_serializable:
        txt_path = output_dir / f'{suffix}_knowledge_docs.txt'
        with open(txt_path, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("DOMAIN KNOWLEDGE EXTRACTED BY LLM\n")
            f.write("=" * 80 + "\n\n")
            for idx, doc in enumerate(knowledge_docs_serializable, 1):
...
def persist_stage_outputs(global_state, stage_name):
    """Export artifacts after each stage automatically."""
    try:
        save_global_state_to_json(global_state, stage_name=stage_name)
    except Exception as exc:
        logger.warning(f"Failed to persist state for stage '{stage_name}': {exc}")


def run_prepare_stage(args, show_header=True):
    if show_header:
        logger.header("Causal-Copilot Analysis Session")
    ...
    Path(global_state.user_data.output_graph_dir).mkdir(parents=True, exist_ok=True)
    Path(global_state.user_data.output_report_dir).mkdir(parents=True, exist_ok=True)
...
def run_execute_stage(args, global_state, show_header=True):
    if show_header:
        logger.header("Causal-Copilot Execution Stage")
    ...
    if global_state.statistics.time_series and global_state.results.lagged_graph is not None:
        ...
        summary_graph = np.any(converted_graph, axis=0).astype(int)
        _ = my_visual_initial.plot_pdag(summary_graph, f'{global_state.algorithm.selected_algorithm}_initial_graph_summary.svg', pos=pos_est)
        global_state.results.raw_pos = pos_est
    else:
    ...
    if getattr(global_state.results, 'raw_edges', None) is None and global_state.results.converted_graph is not None:
...
def run_refine_stage(args, global_state, show_header=True):
    if show_header:
        logger.header("Causal-Copilot Refinement Stage")

    logger.section("Graph Evaluation")
    my_report = Report_generation(global_state, args)

    if global_state.results.raw_edges is None and global_state.results.converted_graph is not None:
        global_state.results.raw_edges = convert_to_edges(
            global_state.algorithm.selected_algorithm,
            global_state.user_data.processed_data.columns,
            global_state.results.converted_graph
    ...
    pos_est = global_state.results.raw_pos
    if pos_est is None and global_state.results.converted_graph is not None:
        pos_est = my_visual_revise.get_pos(global_state.results.converted_graph)
        global_state.results.raw_pos = pos_est

    if global_state.results.revised_graph is not None:
        _ = my_visual_revise.plot_pdag(
...
def main(args):
    ...
    if stage in ('full', 'prepare'):
        global_state = run_prepare_stage(args, show_header=True)
        if args.save_state:
            save_global_state(global_state, args.save_state)
        persist_stage_outputs(global_state, 'prepare')
        if stage == 'prepare':
            return report, global_state

    if stage in ('full', 'execute'):
        if global_state is None:
            if not args.resume_state:
                raise ValueError("--resume-state is required when running the execute stage separately")
            global_state = load_global_state_from_file(args.resume_state)
        global_state = run_execute_stage(args, global_state, show_header=(stage != 'full'))
        if args.save_state:
            save_global_state(global_state, args.save_state)
        persist_stage_outputs(global_state, 'execute')
        if stage == 'execute':
            return report, global_state

    if stage in ('full', 'refine'):
        if global_state is None:
            if not args.resume_state:
                raise ValueError("--resume-state is required when running the refine stage separately")
            global_state = load_global_state_from_file(args.resume_state)
        report, global_state = run_refine_stage(args, global_state, show_header=(stage != 'full'))
        if args.save_state:
            save_global_state(global_state, args.save_state)
        persist_stage_outputs(global_state, 'refine')
