...
class Judge(object):
    ...
    def quality_judge(self, data, full_graph, algorithm, hyperparameters, knowledge_docs, boot_num, prompt_type, voting_num):
        ...
        # print("Warning: Skipping cycle check due to LLM parsing issue")

        return {}, bootstrap_check_dict, boot_probability, llm_pruning_record, revised_graph, bk


    def bootstrap_analysis_only(self, global_state):
        """
        Phase 2: 脰禄脰麓脨脨bootstrap脥鲁录脝路脰脦枚拢卢虏禄碌梅脫脙LLM
        脫脙脫脷路镁脦帽脝梅露脣脌毛脧脽录脝脣茫
        """
        from postprocess.judge_functions import bootstrap, bootstrap_recommend
        from causallearn.utils.PCUtils.BackgroundKnowledge import BackgroundKnowledge
        
        logger.detail("Starting bootstrap sampling...")
        ...
        # 1. Bootstrap虏脡脩霉潞脥脥鲁录脝虏芒脢脭
        edge_recom, boot_probability = bootstrap(
            data=data, 
        ...
        logger.detail(f"Bootstrap completed: {boot_num} iterations")
        
        # 2. Bootstrap脥脝录枚拢篓脥鲁录脝虏驴路脰拢漏
        bk = BackgroundKnowledge()
        revised_graph = full_graph.copy()
        bootstrap_check_dict = bootstrap_recommend(full_graph, boot_probability)
        ...
        # 卤拢麓忙陆谩鹿没碌陆global_state
        global_state.results.bootstrap_probability = boot_probability
        global_state.results.bootstrap_check_dict = bootstrap_check_dict
        global_state.results.revised_graph = revised_graph  # Bootstrap脨脼脮媒潞贸碌脛脥录
        global_state.results.prior_knowledge = bk
        
        logger.detail("Bootstrap analysis completed")
        logger.detail(f"Bootstrap probability matrix shape: {boot_probability.shape}")
        logger.detail(f"High probability non-exist edges: {len(bootstrap_check_dict['high_prob_edges']['non-exist'])}")
        logger.detail(f"Low probability exist edges: {len(bootstrap_check_dict['low_prob_edges']['exist'])}")
        
        return global_state


    def llm_revision_only(self, global_state):
        """
        Phase 3: 脰禄脰麓脨脨LLM脥录脨脼脮媒拢卢录脵脡猫bootstrap脪脩脥锚鲁脡
        脫脙脫脷脨茅脛芒禄煤露脣脭脷脧脽LLM碌梅脫脙
        """
        from postprocess.judge_functions import llm_evaluation_new, kci_pruning, check_cycle
        from postprocess.visualization import convert_to_edges
        from causallearn.utils.PCUtils.BackgroundKnowledge import BackgroundKnowledge
        
        logger.detail("Loading bootstrap results for LLM revision...")
        
        # 录矛虏茅bootstrap陆谩鹿没脢脟路帽麓忙脭脷
        if not hasattr(global_state.results, 'bootstrap_probability'):
            raise ValueError("Bootstrap probability not found! Please run Phase 2 first.")
        if not hasattr(global_state.results, 'bootstrap_check_dict'):
            raise ValueError("Bootstrap check dict not found! Please run Phase 2 first.")
        
        # 禄帽脠隆bootstrap碌脛陆谩鹿没拢篓Phase 2脪脩脥锚鲁脡拢漏
        data = global_state.user_data.processed_data
        revised_graph = global_state.results.revised_graph.copy()  # 脢鹿脫脙bootstrap脨脼脮媒潞贸碌脛脥录
        bootstrap_check_dict = global_state.results.bootstrap_check_dict
        
        # 脰脴脨脗麓麓陆篓BackgroundKnowledge拢篓脠莽鹿没脙禄脫脨卤拢麓忙拢漏
        if not hasattr(global_state.results, 'prior_knowledge'):
            bk = BackgroundKnowledge()
        else:
            bk = global_state.results.prior_knowledge
        
        fixed_pairs = []
        
        ############ Edge Pruning with LLM ############
        logger.detail("Calling LLM for graph refinement...")
        
        revised_edges_dict = convert_to_edges(
            global_state.algorithm.selected_algorithm, 
            global_state.user_data.processed_data.columns, 
            revised_graph
        )
        
        direct_dict, forbid_dict = llm_evaluation_new(
            data, 
            self.args, 
            revised_edges_dict, 
            global_state.results.bootstrap_probability, 
            bootstrap_check_dict, 
            prompt_type='cot_all_relation',  # 脛卢脠脧prompt脌脿脨脥
            voting_num=1  # 脛卢脠脧voting脢媒
        )
        ...
        logger.detail(f"LLM suggested {len(direct_dict)} directed edges")
        logger.detail(f"LLM suggested {len(forbid_dict)} forbidden edges")
        ...
        ############ Edge Pruning with KCI ############
        logger.detail("Applying KCI-based pruning...")
        kci_forbid_dict = kci_pruning(global_state.user_data.processed_data, revised_graph)
        logger.debug(f"KCI forbid dictionary: {len(kci_forbid_dict)} entries", "KCI")
        ...
        ########### Check Cycles ##########
        logger.detail("Checking for cycles...")
        revised_graph = check_cycle(self.args, data, revised_graph)
        
        # 卤拢麓忙脳卯脰脮陆谩鹿没
        global_state.results.llm_errors = llm_pruning_record
        global_state.results.revised_graph = revised_graph
        global_state.results.prior_knowledge = bk
        
        logger.detail("LLM graph revision completed")
