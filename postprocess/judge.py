...
class Judge(object):
    ...
    def bootstrap_analysis_only(self, global_state):
        """
        Phase 2: 只执行bootstrap统计分析，不调用LLM
        用于服务器端离线计算
        """
        ...
        global_state.results.revised_graph = revised_graph  # Bootstrap修正后的图
        global_state.results.prior_knowledge = bk
        
        logger.detail("Bootstrap analysis completed")
        
        # 处理boot_probability可能是字典或数组
        if isinstance(boot_probability, dict):
            logger.detail(f"Bootstrap probability entries: {len(boot_probability)} edges")
        else:
    ...
    def llm_revision_only(self, global_state):
        """
        Phase 3: 只执行LLM图修正，假设bootstrap已完成
        用于虚拟机端在线LLM调用
        """
        ...
        # 获取bootstrap的结果（Phase 2已完成）
        data = global_state.user_data.processed_data
        revised_graph = global_state.results.revised_graph.copy()  # 使用bootstrap修正后的图
        bootstrap_check_dict = global_state.results.bootstrap_check_dict
        
        # 重新创建BackgroundKnowledge（如果没有保存）
        ...
        direct_dict, forbid_dict = llm_evaluation_new(
            ...
            prompt_type='cot_all_relation',  # 默认prompt类型
            vote_num=1  # 默认voting数
        )
