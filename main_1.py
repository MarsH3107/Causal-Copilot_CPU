...
def export_bootstrap_results(global_state):
    ...
    bootstrap_data = {
        'bootstrap_probability': global_state.results.bootstrap_probability.tolist(),
        'bootstrap_check_dict': {
            str(k): v for k, v in global_state.results.bootstrap_check_dict.items()
        },
        'n_bootstraps': getattr(global_state.algorithm.algorithm_arguments, 'n_bootstraps', 100),
        'timestamp': str(pd.Timestamp.now())
    }
    ...
    np.save(npy_path, global_state.results.bootstrap_probability)
    
    logger.success(f"✅ Bootstrap results saved:")
    logger.detail(f"   JSON: {bootstrap_path.name}")
    logger.detail(f"   NumPy: {npy_path.name}")
    logger.detail(f"   Confidence edges: {(global_state.results.bootstrap_probability > 0.5).sum()}")
